...
for i in range(0,1):      # 게시물 크롤링 횟수 시작 0 , 게시판 번호 약 36738 x 7 = 약 25만 페이지
    ...
    fileName = 'hidocHealtystory' + str(fileNo) + '.json'
    ...
    for page in range(startNo,endNo):             #페이지 갯수 설정 시작~끝-1
        ...
        for info in find_str.find_all('li', attrs={'class':'news_item'}): # li 부분 핵심 내용 탐색       
            nUrl = contentsUrl + str(info.a['href']).strip()    # 공백문자처리후 url가져오기
            print(nUrl)
    ...
    # file.write(json.dumps(tempList_nUrl,ensure_ascii=False,indent='\t'))    # 주소목록을 json파일로 만들기
    # file.close()    # 파일 닫기
    ...
    for contentsAddr in tempList_nUrl:    # 컨텐츠 페이지 주소 (coontentsAddr) # ID로 활용 
        ...
        try:
            ...
            ### 뉴스내용 탐색 및 저장 ###
            # tag_search  = soup.find('div', attrs={'class': 'ContentCss'})  # 컨텐츠 목록내용 가져오기
            tag_search = soup.find('div', attrs={'class': 'article_body'}) 
            # print(tag_search)
